...
public class UserDAO {
	...
	/**
	 * This method will check if the id exist in database if don't i'll register the user now if do it'll update
	 * 
	 * @param user
	 */
	 ...
	 * 
	 * @param id Integer value that represent the id user searched
	...
	public User authentication(User user){
		String sql = "SELECT name,username,password from usuario where username = ? and password = ?";
		...
		try(PreparedStatement prepare = con.prepareStatement(sql)){
			...
			if(result.next()){
				...
				getUser.setPassword(result.getString("username"));
				return getUser;
			}
